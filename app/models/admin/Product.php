...
class Product extends AppModel
{
    ...
    {
        return R::getAll("SELECT p.*, pd.title FROM product p JOIN product_description pd on p.id = pd.product_id WHERE pd.language_id = ? LIMIT $start, $per_page", [$lang['id']]);
    }
    ...
    public function getProduct($id): array|false
    {
        ...
        if ($product[$key]['is_download']) {
            $download_info = self::getProductDownload($id);
